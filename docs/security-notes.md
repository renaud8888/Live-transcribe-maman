...
Le navigateur appelle `/session`. Le serveur vérifie éventuellement `ACCESS_CODE`, puis crée un client secret temporaire auprès d’OpenAI avec :
...
`ACCESS_CODE` n’est pas une sécurité bancaire. C’est seulement une barrière simple pour éviter qu’une personne qui tombe sur l’URL utilise l’app trop facilement.

Pour une sécurité plus forte, il faudrait ajouter une authentification complète, une limitation de débit, et une supervision côté serveur.
